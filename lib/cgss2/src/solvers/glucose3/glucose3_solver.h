#ifndef GLUCOSE3_SOLVER_H
#define GLUCOSE3_SOLVER_H
...
class Glucose3Solver : cgss2::SATsolver{
  Glucose3::Solver solver;
  int topv;

  void intsToLits(const std::vector<int>& lits, Glucose3::vec<Glucose3::Lit>& rLits) {
    rLits.capacity(lits.size());
    for (unsigned i=0;i<lits.size();++i) {
      while ((lits[i]>>1)>topv) {solver.newVar();++topv;}
      rLits.push_(Glucose3::toLit(lits[i]));
    }
  }

  void litsToInts(Glucose3::vec<Glucose3::Lit>& lits, std::vector<int>& rLits, bool neg = false) {
    rLits.resize(lits.size());
    if (!neg)
      for (int i=0; i<lits.size(); ++i) rLits[i]=Glucose3::toInt(lits[i]);
    else
      for (int i=0; i<lits.size(); ++i) rLits[i]=Glucose3::toInt(~lits[i]);
  }

  void lboolsToBools(Glucose3::vec<Glucose3::lbool>& lbools, std::vector<bool>& bools) {
